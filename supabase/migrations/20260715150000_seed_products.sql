...
BEGIN
  ...
  INSERT INTO public.products (slug, name, short_description, description, specs, images, price_ngn, compare_at_price_ngn, deposit_percent, stock, low_stock_threshold, is_active, is_featured, is_subscription, subscription_interval, paystack_plan_code, sku, category_id)
  VALUES ('nx-nvr-16ch-4tb', '16-Channel PoE NVR (4TB)', 'Enterprise-grade 16ch NVR with 4TB', '16-channel enterprise NVR with 16 PoE ports and 4TB storage. Perfect for large residences, offices and estates.', '{"storage": "4TB surveillance HDD", "channels": 16, "poe_ports": 16, "max_resolution": "4K per channel"}'::jsonb, ARRAY['/__l5e/assets-v1/bab25e8a-1b15-4cc9-ae1c-38b573134e4e/nx-nvr-16ch-4tb.jpg']::text[], 580000.0, NULL, 60, 6, 2, TRUE, FALSE, FALSE, NULL, NULL, 'NX-NVR-16CH', (SELECT id FROM public.product_categories WHERE slug = 'nvrs-dvrs'))
  ON CONFLICT (slug) DO UPDATE SET
    name = EXCLUDED.name,
    short_description = EXCLUDED.short_description,
    description = EXCLUDED.description,
  ...
  INSERT INTO public.products (slug, name, short_description, description, specs, images, price_ngn, compare_at_price_ngn, deposit_percent, stock, low_stock_threshold, is_active, is_featured, is_subscription, subscription_interval, paystack_plan_code, sku, category_id)
  VALUES ('wf-solar-kit-4cam', 'Solar Kit — 4-Camera Compound', '4-camera solar kit for a medium compound perimeter.', '4x 4MP solar bullet cameras, a solar-fed 4G router and central battery buffer. Everything you need for a full perimeter without trenching.', '{"tech": {"cameras": 4, "warranty": "2 years"}, "features": ["4x 4MP solar bullet cams", "Central 4G router hub", "Central battery buffer", "Full mounting hardware", "Free remote setup after install"], "use_case": "Medium compounds and farms", "hook_line": "4-camera solar kit for a medium compound perimeter.", "image_brief": "Flat-lay of 4 white solar bullet cameras, a router hub, battery box, cables and brackets on a workbench.", "recommended_for": "Medium compounds"}'::jsonb, ARRAY['/__l5e/assets-v1/09a283d2-245d-4476-9d97-25412ea39b0a/wf-solar-kit-4cam.jpg']::text[], 825000.0, NULL, 50, 6, 5, TRUE, TRUE, FALSE, NULL, NULL, NULL, (SELECT id FROM public.product_categories WHERE slug = 'solar-cctv'))
  ON CONFLICT (slug) DO UPDATE SET
    name = EXCLUDED.name,
    short_description = EXCLUDED.short_description,
    description = EXCLUDED.description,
